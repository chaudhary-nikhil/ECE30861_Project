...
def make_request(endpoint: str, method: str = 'POST', data: Optional[Dict] = None) -> Optional[Dict]:
...
def print_test_results(data: Dict[str, Any], indent: int = 0):
    ...
    for key, value in data.items():
        if key in ['autograder_run_log', 'system_run_log', 'start_time', 'end_time', 'run_time']:
            continue
...
def monitor_runs(wait: bool = True, poll_interval: int = 10):
    ...
    print_info(f"Polling every {poll_interval} seconds for Group {GROUP_NUMBER}... (Press Ctrl+C to stop)\n")
...
def main():
    ...
    parser.add_argument('--monitor', action='store_true', help='Monitor runs (one-time check)')
    ...
    while True:
        ...
        choice = input(f"\n{Colors.OKCYAN}Select an option (1-6): {Colors.ENDC}").strip()
