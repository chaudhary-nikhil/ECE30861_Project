...
def calculate_size_score(model_size_mb: float) -> dict[str, float]:
    ...
    thresholds = {
        'raspberry_pi': {'min': 0, 'max': 200},      # 0-200MB full score, taper to 0 at 1GB+
    ...
    for hardware, threshold in thresholds.items():
        if model_size_mb <= threshold['min']:
            ...
            score = max(0.0, 1.0 - (model_size_mb - threshold['min']) / (threshold['max'] - threshold['min']))
    ...
    try:
        ...
        for root, dirs, files in os.walk(repo_path):
            for file in files:
                ...
                if file in ['pytorch_model.bin', 'tf_model.h5', 'model.safetensors', 'pytorch_model-00001-of-00001.bin']:
                ...
                elif file in ['tokenizer.json', 'vocab.txt', 'tokenizer_config.json', 'tokenizer.model']:
            ...
            try:
                with open(config_path, 'r') as f:
                    ...
                    if 'model_type' in config_data:
                        ...
                        analysis['architecture'] = config_data['architectures'][0] if config_data['architectures'] else 'unknown'
            except Exception as e:
                print(f"Failed to parse config.json: {e}")
        
        print(f"Analysis complete: {analysis['total_size_mb']}MB total, {analysis['weights_size_mb']}MB weights")
...
def score_dataset(url: str) -> ScoreResult:
    ...
    if not match:
        ...
        return ScoreResult(url, UrlCategory.DATASET, 0.0, 10.0, {'error': 'Invalid URL', 'name': 'unknown', 'size_score': size_score})
    ...
    if not data:
        ...
        size_score = calculate_size_score(estimated_size)
        return ScoreResult(url, UrlCategory.DATASET, 0.0, 10.0, {'name': dataset_name, 'fallback': True, 'size_score': size_score})
...
def score_model(url: str) -> ScoreResult:
    ...
    if not match:
        ...
        size_score = calculate_size_score(estimated_size)
        return ScoreResult(url, UrlCategory.MODEL, 0.0, 10.0, {'error': 'Invalid URL', 'name': 'unknown', 'size_score': size_score})
    ...
    if not data:
        ...
        size_score = calculate_size_score(estimated_size)
        return ScoreResult(url, UrlCategory.MODEL, 2.0, 10.0, {'name': model_name, 'fallback': True, 'size_score': size_score})
...
def score_code(url: str) -> ScoreResult:
    ...
    if not match:
        ...
        size_score = calculate_size_score(estimated_size)
        return ScoreResult(url, UrlCategory.CODE, 0.0, 10.0, {'error': 'Invalid URL', 'name': 'unknown', 'size_score': size_score})
    ...
    if not data:
        ...
        size_score = calculate_size_score(estimated_size)
        return ScoreResult(url, UrlCategory.CODE, 2.0, 10.0, {'name': f"{owner}/{repo}", 'fallback': True, 'size_score': size_score})
...
def score_url(url: str, category: UrlCategory) -> ScoreResult:
    ...
    if category == UrlCategory.DATASET:
        ...
        size_score = calculate_size_score(estimated_size)
        return ScoreResult(url, UrlCategory.INVALID, 0.0, 10.0, {'error': 'Invalid category', 'name': 'unknown', 'size_score': size_score})
