...
def calculate_bus_factor_with_timing(url: str, category: UrlCategory, data: Dict[str, Any]) -> tuple:
    ...
    # logger.log_info(f"DEBUG - URL: {url}")
    # logger.log_info(f"DEBUG - Category: {category.name}")
    # logger.log_info(f"DEBUG - Contributors found: {contributor_count}")
    # logger.log_info(f"DEBUG - Contributor list: {contributors[:5]}")  # Show first 5
    # logger.log_info(f"DEBUG - Raw data keys: {list(data.keys())}")
    # logger.log_info("-" * 50)
