...
def score_dataset(url: str) -> ScoreResult:
    ...
    if not data:
        return ScoreResult(url, UrlCategory.DATASET, 0.0, 10.0, {'name': dataset_name, 'fallback': True})
...
def score_model(url: str) -> ScoreResult:
    ...
    if not data:
        return ScoreResult(url, UrlCategory.MODEL, 2.0, 10.0, {'name': model_name, 'fallback': True})
...
def score_code(url: str) -> ScoreResult:
    ...
    if not data:
        return ScoreResult(url, UrlCategory.CODE, 2.0, 10.0, {'name': f"{owner}/{repo}", 'fallback': True})
